#include "myCondTools/GEM/interface/GEMDeadStripSourceHandler.h"
...
popcon::GEMDeadStripSourceHandler::GEMDeadStripSourceHandler( const edm::ParameterSet& ps ):
  m_name( ps.getUntrackedParameter<std::string>( "name", "GEMDeadStripSourceHandler" ) ),
...
popcon::GEMDeadStripSourceHandler::~GEMDeadStripSourceHandler()
{
}

void popcon::GEMDeadStripSourceHandler::getNewObjects()
{
  
  edm::LogInfo( "GEMDeadStripSourceHandler" ) << "[" << "GEMDeadStripSourceHandler::" << __func__ << "]:" << m_name << ": "
                                         << "BEGIN" << std::endl;
  ...
  while(std::getline(maptype, line)){
    GEMDeadStrips::DeadItem itemNew;
  ...
  int difference=1;
  if (difference==1) {
    m_to_transfer.push_back(std::make_pair((GEMDeadStrips*)stripsMap,snc));
  }
}

// // additional work (I added these two functions: ConnectOnlineDB and DisconnectOnlineDB)
void popcon::GEMDeadStripSourceHandler::ConnectOnlineDB( const std::string& connect, const edm::ParameterSet& connectionPset )
{
...
void popcon::GEMDeadStripSourceHandler::DisconnectOnlineDB()
{
  session.close();
}
